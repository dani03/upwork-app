    ...
    <div class="flex flex-col md:flex-row">
            ...
            @foreach (auth()->user()->proposals as $proposal)
                <div class="mb-3 {{ $proposal->validated ? 'text-green-400' : '' }}">
                    ...
                    <span>Lettre de motivation : <span
                            class="font-semibold">{{ $proposal->coverLetter->content ?? 'nope' }}</span></span>
                </div>
            @endforeach
        </section>
        ...
        <section class="text-sm text-gray-700 w-full">
            <h2 class="text-xl my-2">
                ...
                Vos missions ({{ auth()->user()->jobs()->count() ?? '0' }})
            </h2>
            @foreach (auth()->user()->jobs as $job)
                <div class="mb-3">
                    <span class="block font-semibold">
                        {{ $job->title }} ({{ $job->proposals->count() }} @choice('proposition|propositions',
                        $job->proposals)) :</span>
                    </span>
                    <ul class="list-none ml-4">
                        @foreach ($job->proposals as $proposal)
                            <li class="mt-2">• "{{ $proposal->coverLetter->content ?? 'no contenu' }}" par
                                <strong>
                                    {{ $proposal->user->name }}
                                </strong>
                            </li>
